...
{#if unit.type=='transclusion'}
<Transclusion {active} {addr} text={unit.text} {depth}/>{:else if unit.type=='html'}
{@html unit.text}
{:else if unit.offtag}
<Offtag text={unit.text} offtag={unit.offtag} {addr} {active}/>
{:else if unit.type=='offtext'}
<Offtext text={unit.text} {addr}/>
{:else}
<Textout text={unit.text.replace(/\^\^/g,'^').replace(/\n/g,"<br/>")} tappable={true}/>
{/if}
{/each}{#if text.length}
<NestedLineInfo {active} {addr} {lineinfo}/>
<Parallel {addr}  {active}/>{/if}
